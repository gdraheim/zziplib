...
RUN zypper install -r repo-oss -y gcc11 zlib-devel python3 cmake unzip zip gzip tar  libSDL2-devel
...
RUN mkdir src/build
RUN cd src/build && cmake .. -DCMAKE_C_COMPILER=/usr/bin/gcc-11 -DZZIP_TESTCVE=OFF

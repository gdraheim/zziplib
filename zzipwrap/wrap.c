...
ZZIP_memory int
zzip_memory_read(int fd, char* buf, size_t count)
{
    int requested_block;
    int transfer_start;
    int transfer_bytes;
    ...
    while (bytes_to_write > 0) {
        ...
        if (zzip_memory_bufferblock != requested_block) {
            ...
            if (zzip_memory_callback) {
                zzip_memory_callback(zzip_memory_buffer, zzip_memory_blocksize,
...
ZZIP_memory int
zzip_memory_open(const char* pathname, int flags)
{
    int         fd;
...
ZZIP_memory off_t
zzip_memory_lseek(int fildes, off_t offset, int whence)
{
    /* Check file descriptor */
